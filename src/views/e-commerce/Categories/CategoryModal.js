...
const CategoryModal = ({open, modal, handleModal, setUpdate, edit, category}) => {
  ...
  useEffect(() => {
      if ((nameIsChanged || orderIsChanged || visibilityIsChanged || imageIsChanged) 
          && nameValue.trim().length && imageValue !== null) setShowButton(true)
      else setShowButton(false)
  }, [nameValue, imageValue, nameIsChanged, orderIsChanged, visibilityIsChanged, imageIsChanged])
  ...
  const handleOnSubmit = async e => {
    ...
    if (!modal.edit || (nameIsChanged && nameValue !== "")) formData.append('name', nameValue)
    if (!modal.edit || visibilityIsChanged) formData.append('visibility', e.target.visibility.checked)
    if (!modal.edit || orderIsChanged) formData.append('order', e.target.order.value)
    if (!modal.edit || (imageIsChanged && e.target.image.files.length)) {
        ...
        <ModalBody className='px-sm-5 mx-50 pb-5'>
          ...
          <Row tag='form' className='gy-1 gx-2 mt-75' onSubmit={e => handleOnSubmit(e)}>
              ...
              <div className='d-flex align-items-center'>
                <div className='form-switch w-100'>
                  <Input 
                    ...
                    name='visibility' 
                    id='visibility'
                    defaultChecked={visibilityValue}
                    onChange={(e) => { setVisibilityValue(e.target.checked); setVisibilityIsChanged(true) }}
                  />
            ...
            <Col xs={12}>
              ...
              <InputPreview 
                inputName='image'
                onChange={ file => { setImageValue(file); setImageIsChanged(true) } }
                {...(category ? {imageSrc: imageValue} : {})} 
              />
            </Col>
